...
export class Sakura {
  constructor (selector, options) {
    ...
    const extend = function extend (originalObj, newObj) {
    ...
    setInterval(function () {
      ...
      const _iterator = _createForOfIteratorHelper(_this.petalsWeak)
      let _step

      try {
        for (_iterator.s(); !(_step = _iterator.n()).done;) {
          const _step$value = _slicedToArray(_step.value, 2)
          const _key = _step$value[0]
      ...
      for (let _i = 0, _keysForRemove = keysForRemove; _i < _keysForRemove.length; _i++) {
        const key = _keysForRemove[_i]
    ...
    this.el.style.position = 'relative' // Random array element
    ...
    this.createPetal = function () {
      ...
      const fallTime = (document.documentElement.clientHeight * 0.007 + Math.round(Math.random() * 5)) * _this.settings.fallSpeed // Create animations

      const animationsArr = ['fall '.concat(fallTime, 's linear 0s 1'), ''.concat(blowAnimation, ' ').concat((fallTime > 30 ? fallTime : 30) - 20 + randomInt(0, 20), 's linear 0s infinite'), ''.concat(swayAnimation, ' ').concat(randomInt(2, 4), 's linear 0s infinite')]
      ...
      const height = randomInt(_this.settings.minSize, _this.settings.maxSize)
      const width = height - Math.floor(randomInt(0, _this.settings.minSize) / 3) // Get a random color.

      const color = randomArrayElem(_this.settings.colors)
  ...
  stop () {
    ...
    const graceful = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : false
    const animId = this.el.dataset.sakuraAnimId
    ...
    if (!graceful) {
      setTimeout(function () {
        const petals = document.getElementsByClassName(_this2.settings.className)
